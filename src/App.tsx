...
import {
  Moon, Sun, Menu, X, Mail, Phone, MapPin, Linkedin,
  ChevronDown, Cloud, Server, Shield, Container,
  GitBranch, Network, Award, GraduationCap,
  Code2, Layers, HardDrive,
  ArrowRight, Globe, Zap, BookOpen, Languages
} from 'lucide-react';
...
export default function App() {
  ...
  const navIds = ['home', 'about', 'skills', 'experience', 'projects', 'courses', 'education', 'contact'];
  ...
  const skillCategories = [
    {
      ...
      skills: ['VMware Workstation', 'Oracle VM VirtualBox'],
      color: 'from-brand-500 to-terminal-cyan',
    },
    {
      icon: <GitBranch className="w-6 h-6" />,
      title: t.skills.categories[1].title,
      skills: ['Ansible', 'Jenkins', 'Git', 'GitHub', 'Docker', 'Kubernetes', 'Terraform', 'Prometheus'],
      color: 'from-terminal-amber to-brand-600',
    },
    {
      icon: <Layers className="w-6 h-6" />,
      title: t.skills.categories[2].title,
      skills: ['Angular', 'Laravel', 'Bootstrap', 'Spring Boot'],
      color: 'from-brand-600 to-terminal-cyan',
    },
    {
    ...
    },
    {
      ...
      skills: ['VLAN', 'Routing', 'OSI model', 'DNS', 'FTP', 'NFS', 'SAMBA', 'DHCP', 'TCP/IP', 'SNMP'],
      color: 'from-terminal-cyan to-brand-600',
    },
    {
      icon: <Code2 className="w-6 h-6" />,
      title: t.skills.categories[5].title,
      skills: ['C', 'PHP', 'Java', 'Python'],
      color: 'from-terminal-amber to-brand-500',
    },
    {
    ...
    },
    {
      ...
      skills: ['UML'],
      color: 'from-brand-600 to-terminal-amber',
    },
  ];

  const experienceTags = [
    ['Machine Learning', 'Security Logs', 'Python', 'Wazuh'],
    ['CI/CD', 'Docker', 'Automation', 'Delivery Tracking'],
    ['Laravel', 'MySQL', 'Web'],
    ['Node.js', 'Angular', 'Agri Tech'],
  ...
  const projectMeta = [
    { tags: ['Jenkins', 'AKS', 'Terraform', 'SonarCloud', 'Nexus', 'Trivy', 'Grafana', 'Prometheus'], icon: <GitBranch className="w-8 h-8" />, gradient: 'from-brand-700 to-terminal-cyan' },
    { tags: ['OpenStack', 'Kubernetes', 'Ansible', 'Heat', 'Neutron', 'Magnum'], icon: <Cloud className="w-8 h-8" />, gradient: 'from-terminal-amber to-brand-600' },
    { tags: ['Angular', 'Spring Boot', 'Microservices', 'Authentication', 'Cloud'], icon: <Server className="w-8 h-8" />, gradient: 'from-brand-500 to-terminal-amber' },
    { tags: ['C', 'Glade', 'Linux', 'Desktop'], icon: <Layers className="w-8 h-8" />, gradient: 'from-brand-600 to-brand-800' },
  ];
  ...
  const courses = t.courses.items.map((course) => ({
    ...course,
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-b border-gray-200/60 dark:border-gray-800/60 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
          ...
          <button onClick={() => scrollTo('home')} className="flex items-center gap-2 group">
            <span className="w-9 h-9 rounded-lg bg-gradient-to-br from-brand-500 to-terminal-cyan flex items-center justify-center text-white font-bold text-sm group-hover:scale-110 transition-transform ring-1 ring-brand-500/30 shadow-md">HA</span>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="animate-fade-in">
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold mb-4 leading-tight">
                <span className="text-gray-900 dark:text-white">{t.hero.firstName}</span><br />
              ...
              <div className="terminal-window animate-pulse-glow">
                ...
                <div className="terminal-header justify-between">
                  ...
                  <div className="flex items-center gap-1 text-[10px] font-mono">
                    <span className="px-2.5 py-1 rounded-t bg-gray-800 text-terminal-cyan border-b-2 border-terminal-cyan">☸ cluster</span>
                    <span className="px-2.5 py-1 rounded-t text-gray-500 hover:text-gray-300">🐳 containers</span>
                    <span className="px-2.5 py-1 rounded-t text-gray-500 hover:text-gray-300">☁ cloud</span>
                  </div>
                </div>

                <div className="terminal-body text-gray-300 space-y-3">
                  {/* Cluster status */}
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-gray-500 uppercase tracking-wider">Kubernetes Cluster</p>
                    <span className="flex items-center gap-1.5 text-xs"><span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /> Healthy</span>
                  </div>

                  {/* Nodes */}
                  <div className="bg-gray-900/80 rounded-lg p-3 border border-gray-800">
                    <p className="text-[10px] text-gray-500 mb-2 uppercase tracking-wider">Nodes</p>
                    <div className="space-y-1.5 text-xs font-mono">
                      <div className="flex items-center justify-between">
                        <span><span className="text-terminal-cyan">⬢</span> master-01</span>
                        <span className="text-green-400">Ready</span>
                        <span className="text-gray-500">4 CPU · 16Gi</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span><span className="text-terminal-cyan">⬢</span> worker-01</span>
                        <span className="text-green-400">Ready</span>
                        <span className="text-gray-500">8 CPU · 32Gi</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span><span className="text-terminal-cyan">⬢</span> worker-02</span>
                        <span className="text-green-400">Ready</span>
                        <span className="text-gray-500">8 CPU · 32Gi</span>
                      </div>
                    </div>
                  ...
                  {/* Pods / Containers */}
                  <div className="bg-gray-900/80 rounded-lg p-3 border border-gray-800">
                    <p className="text-[10px] text-gray-500 mb-2 uppercase tracking-wider">Running Pods</p>
                    <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                      {[
                        { name: 'frontend', status: '3/3', color: 'text-green-400' },
                        { name: 'api-gateway', status: '2/2', color: 'text-green-400' },
                        { name: 'ml-detector', status: '1/1', color: 'text-green-400' },
                        { name: 'wazuh-agent', status: '2/2', color: 'text-green-400' },
                        { name: 'prometheus', status: '1/1', color: 'text-green-400' },
                        { name: 'grafana', status: '1/1', color: 'text-green-400' },
                      ].map((pod) => (
                        <div key={pod.name} className="flex items-center justify-between bg-gray-950/60 rounded px-2 py-1">
                          <span className="text-gray-300 truncate">🐳 {pod.name}</span>
                          <span className={pod.color}>{pod.status}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Pipeline */}
                  <div className="bg-gray-900/80 rounded-lg p-3 border border-gray-800">
                    <p className="text-[10px] text-gray-500 mb-2 uppercase tracking-wider">CI/CD Pipeline</p>
                    <div className="flex items-center gap-1 text-[11px] font-mono">
                      <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">✓ Build</span>
                      <span className="text-gray-600">→</span>
                      <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">✓ Test</span>
                      <span className="text-gray-600">→</span>
                      <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">✓ Scan</span>
                      <span className="text-gray-600">→</span>
                      <span className="px-2 py-0.5 rounded bg-terminal-cyan/20 text-terminal-cyan animate-pulse">⟳ Deploy</span>
                    </div>
                  </div>

                  {/* Cloud metrics bar */}
                  <div className="flex items-center justify-between text-[10px] text-gray-500 pt-1">
                    <span>☁ OpenStack &nbsp;|&nbsp; ☸ v1.28 &nbsp;|&nbsp; 🐳 24 containers</span>
                    <span className="text-terminal-green animate-pulse">● live</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="glass-card p-8 lg:col-span-1 flex flex-col items-center text-center">
              ...
              <div className="flex gap-3 mb-6">
                <a href="https://www.linkedin.com/in/hamdi-alaa-eddin-538898229/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center hover:bg-brand-500 hover:text-white transition-all">
                  <Linkedin className="w-5 h-5" />
                </a>
                <a href="tel:+21625968838" className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center hover:bg-brand-500 hover:text-white transition-all">
                  <Phone className="w-5 h-5" />
                </a>
                <a href="mailto:alaa.hamdi01@gmail.com" className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center hover:bg-brand-500 hover:text-white transition-all">
                  <Mail className="w-5 h-5" />
                </a>
              </div>
              <div className="w-full space-y-3 text-sm text-left">
                <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                  <MapPin className="w-4 h-4 text-brand-500 flex-shrink-0" /> Tunis, Tunisia
                </div>
                <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                  <Phone className="w-4 h-4 text-brand-500 flex-shrink-0" /> +216 25 968 838
                </div>
                <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
      ...
      <Section id="courses" className="py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="section-heading text-gray-900 dark:text-white"><span className="gradient-text">{t.courses.heading}</span></h2>
            <div className="w-20 h-1 bg-gradient-to-r from-brand-500 to-terminal-cyan mx-auto rounded-full mt-4" />
          </div>
            ...
            {courses.map((course, i) => (
              <div key={i} className="glass-card p-8 flex items-center gap-6 hover:shadow-xl transition-shadow hover:scale-[1.02]">
                ...
                <div>
                  ...
                  <p className="text-brand-600 dark:text-brand-400 font-semibold">{course.issuer}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{course.date}</p>
                </div>
              ...
              <div key={i} className="glass-card p-8 sm:p-10 hover:shadow-xl transition-shadow">
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 font-mono text-sm mt-2">{edu.period}</p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {edu.tags.map((tag, idx) => (
                        <span key={idx} className="skill-badge text-xs">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid sm:grid-cols-2 gap-6 mb-10">
              <a href="mailto:alaa.hamdi01@gmail.com" className="glass-card glow-border p-6 flex items-center gap-5 hover:scale-[1.03] transition-all group">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 break-all">alaa.hamdi01@gmail.com</p>
                </div>
              </a>

              <a href="tel:+21625968838" className="glass-card glow-border p-6 flex items-center gap-5 hover:scale-[1.03] transition-all group">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-brand-600 to-brand-500 flex items-center justify-center text-white flex-shrink-0 group-hover:scale-110 transition-transform">
                  <Phone className="w-6 h-6" />
                </div>
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">{t.contact.phone}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">+216 25 968 838</p>
                </div>
              </a>
              ...
              <div className="glass-card glow-border p-6 flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">Tunis, Tunisia</p>
                </div>
              </div>

              <a href="https://www.linkedin.com/in/hamdi-alaa-eddin-538898229/" target="_blank" rel="noopener noreferrer" className="glass-card glow-border p-6 flex items-center gap-5 hover:scale-[1.03] transition-all group">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-brand-600 to-brand-500 flex items-center justify-center text-white flex-shrink-0 group-hover:scale-110 transition-transform">
                  <Linkedin className="w-6 h-6" />
                </div>
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">LinkedIn</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">hamdi-alaa-eddin</p>
                </div>
              </a>
            </div>
            ...
            <div className="terminal-window max-w-2xl mx-auto">
              <div className="terminal-header justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="terminal-dot bg-yellow-500" />
                  <span className="terminal-dot bg-green-500" />
                  <span className="ml-2 text-xs text-gray-500 font-mono">hamdi@cloud-console</span>
                </div>
                <span className="text-[10px] text-terminal-green font-mono animate-pulse">● connected</span>
              </div>
              <div className="terminal-body text-gray-300 space-y-2">
                <p className="text-xs text-gray-500">$ kubectl apply -f collaboration.yml</p>
                <div className="bg-gray-900/80 rounded-lg p-3 border border-gray-800 space-y-1.5 text-xs font-mono">
                  <p><span className="text-terminal-cyan">kind:</span> <span className="text-white">Collaboration</span></p>
                  <p><span className="text-terminal-cyan">metadata.name:</span> <span className="text-terminal-amber">"let-us-build-together"</span></p>
                  <p><span className="text-terminal-cyan">spec.engineer:</span> <span className="text-white">Hamdi Alaa Eddin</span></p>
                  <p><span className="text-terminal-cyan">spec.email:</span> <span className="text-white">alaa.hamdi01@gmail.com</span></p>
                  <p><span className="text-terminal-cyan">spec.replicas:</span> <span className="text-terminal-green">∞</span></p>
                </div>
                <p className="text-green-400 text-xs">✓ collaboration/let-us-build-together created</p>
                <p className="text-xs"><span className="text-gray-500">$</span> <span className="animate-terminal-blink text-terminal-green">▎</span></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer className="py-10 px-4 sm:px-6 lg:px-8 border-t border-gray-200 dark:border-gray-800">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-terminal-cyan flex items-center justify-center text-white font-bold text-xs ring-1 ring-brand-500/30 shadow-md">HA</span>
